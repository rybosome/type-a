...
export function Of<T extends Typeable, R = T>(opts?: {
  ...
  /* -------------------------------------------------------------- */
  /*
   * When the caller did NOT provide an explicit `is` constraint we try to
   * auto-generate one for *literal* unions (e.g. "draft" | "published").
   *
   * Runtime type information is lost after compilation so we rely on a
   * best-effort static-code inspection of the *call-site’s source file* to
   * extract the union members.  While not fool-proof, the heuristic works
   * reliably for the simple one-line patterns used throughout this library’s
   * documentation and test-suite:
   *
   *   type Foo = "a" | "b" | "c";
   *   const Model = Schema.from({ state: Of<Foo>() });
   *
   * The implementation is intentionally sandboxed in an IIFE so that any
   * parsing failure simply results in `generatedIs` being `undefined` (we then
   * fall back to accepting the value).  This keeps behaviour backwards-
   * compatible for complex generic types where automatic guards are either
   * impossible or undesirable.
   */

  const generatedIs: LogicalConstraint<NonNullable<T>> | undefined = (() => {
    // Skip when the caller already supplied a validator
    if (opts?.is) return undefined;
    ...
    // Use stack-trace inspection to locate the *ts* file that invoked `Of<T>`.
    const stackLines = new Error().stack?.split("\n") ?? [];
    let callerLine: string | undefined;
    for (const ln of stackLines) {
      console.log("[type-a] stack line", ln);
      if (ln.includes("Of (")) continue; // Skip internal helper frame
      if (ln.includes("at Of")) continue;
      if (ln.includes(__filename)) continue; // skip frames inside this module
      if (ln.trim() === "Error") continue; // skip header
      callerLine = ln;
      break;
    }
    if (!callerLine) return undefined;

    // Extract file path & line number – format:  at <fn> (PATH:LINE:COL)
    const match = callerLine.match(
      /at(?: [^()]+)? \(?([^:\s]+):(\d+):(\d+)\)?/,
    );
    if (!match) return undefined;
    const [, filePathStr, lineStr] = match;
    const filePath = path.resolve(filePathStr);

    // Only attempt when the source exists and is a TypeScript file
    if (!filePath.endsWith(".ts") || !fs.existsSync(filePath)) return undefined;

    // Read the *entire* file so we can parse unions and nearby type aliases
    const src = fs.readFileSync(filePath, "utf8");

    // ------------------------------------------------------------------
    // 1. Extract the generic parameter literal from the call-site line.
    // ------------------------------------------------------------------

    const lineNum = Number(lineStr) - 1; // 0-based index
    const lines = src.split(/\r?\n/);
    const callLine = lines[lineNum] ?? "";

    // Handle simple single-line invocations:  Of<"a" | "b">()
    let genericArg = (() => {
      const m = callLine.match(/Of<([^>]+)>/);
      return m ? m[1].trim() : undefined;
    })();

    // If not found, try concatenating following lines until we encounter ">"
    if (!genericArg) {
      let i = lineNum;
      let acc = "";
      while (i < lines.length && !acc.includes(">")) {
        acc += lines[i];
        i += 1;
      }
      const m = acc.match(/Of<([^>]+)>/);
      if (m) genericArg = m[1].trim();
    }

    if (!genericArg) {
      if (process.env.DEBUG_LITERAL_GUARD) {
        console.warn(
          "[type-a] failed to extract generic argument from call-site",
        );
      }
      return undefined;
    }
    ...
    // ------------------------------------------------------------------
    // 2. Resolve type alias when the argument is *not* a literal itself.
    // ------------------------------------------------------------------

    let unionExpr = genericArg;

    // When the argument is a single identifier (e.g. PostState) look for a
    // matching `type` alias in the same file.
    if (/^[A-Za-z0-9_$]+$/.test(genericArg)) {
      const aliasRe = new RegExp(
        `type\\s+${genericArg}\\s*=\\s*([^;]+);?`,
        "m",
      );
      const m = src.match(aliasRe);
      if (m) unionExpr = m[1].trim();
    }

    // Bail when the expression still contains invalid characters (e.g. nested
    // generics, conditionals, etc.) – those cases are out-of-scope for this
    // lightweight parser.
    if (/[^|\s0-9a-zA-Z_'"`-]/.test(unionExpr)) {
      if (process.env.DEBUG_LITERAL_GUARD) {
        console.warn(
          `[type-a] union expression contains unsupported tokens: ${unionExpr}`,
        );
      }
      return undefined;
    }
    ...
    // ------------------------------------------------------------------
    // 3. Tokenise the union and parse each literal.
    // ------------------------------------------------------------------

    if (process.env.DEBUG_LITERAL_GUARD) {
      console.log("[type-a] unionExpr =", unionExpr);
    }

    const allowed: unknown[] = [];
    for (const rawTok of unionExpr.split("|")) {
      const tok = rawTok.trim();
      if (!tok) continue;

      // String literal – supports single/double/backtick quotes
      const strMatch = tok.match(/^['"`](.*)['"`]$/);
      if (strMatch) {
        allowed.push(strMatch[1]);
        continue;
      }

      // Boolean literal
      if (tok === "true" || tok === "false") {
        allowed.push(tok === "true");
        continue;
      }

      // Numeric literal (int or float)
      if (/^-?\d+(?:\.\d+)?$/.test(tok)) {
        allowed.push(Number(tok));
        continue;
      }

      // Anything else (e.g. bigint, template-literal types, etc.) – abort.
      return undefined;
    }

    if (allowed.length === 0) return undefined;

    const set = new Set(allowed as NonNullable<T>[]);
    if (process.env.DEBUG_LITERAL_GUARD) {
      console.log("[type-a] auto-literal-guard ->", allowed);
    }

    return ((val: NonNullable<T>) =>
      set.has(val)
        ? true
        : `${JSON.stringify(val)} is not one of ${[...set]
            .map((v) => JSON.stringify(v))
            .join(" | ")}`) as LogicalConstraint<NonNullable<T>>;
  })();

  const base = {
    value: undefined as T | undefined,
    ...(generatedIs ? { is: generatedIs } : opts?.is ? { is: opts.is } : {}),
