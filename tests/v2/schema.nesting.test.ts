...
import { Schema, one, many, nonEmpty, atLeast } from "@rybosome/type-a";
...
describe("Schema nesting", () => {
  class Address extends Schema.from({
    firstLine: one().of<string>({ is: nonEmpty }),
    secondLine: one().of<string | undefined>({}),
    city: one().of<string>({ is: nonEmpty }),
    state: one().of<string>({ is: nonEmpty }),
    zip: one().of<number>({ is: atLeast(10000) }),
  }) {}
  ...
  it("supports deeply-nested structures", () => {
    class Company extends Schema.from({
      name: one().of<string>({ is: nonEmpty }),
      hq: one(Address).of<Address>({}),
    }) {}

    class Account extends Schema.from({
      owner: one(User).of<User>({}),
      employer: one(Company).of<Company>({}),
    }) {}
  ...
  it("supports type-composition", () => {
    ...
    class WorkHistory extends Schema.from({
      employers: many(Company).of<Company[]>({}),
    }) {}
