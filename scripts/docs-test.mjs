 ...
 * @returns {string[]}
 */
function extractTestBlocks(markdown) {
  // Regex breakdown:
  // ```            opening fence
  // (?:ts|typescript)[\t ]+test   language tag (ts|typescript) followed by whitespace and the word test
  // [^\n]*         allow the rest of the first line until newline (to ignore additional attrs)
 ...
 * @param {Array<{file:string, blocks:string[]}>} docs
 * @returns {Promise<string[]>}
 */
async function generateTests(docs) {
  const generated = [];
  let counter = 0;
  for (const { file, blocks } of docs) {
    ...
    for (const block of blocks) {
      ...
      const codeLines = lines.filter((l) => !/^\s*import\s/.test(l));

      const indented = codeLines.map((line) => `    ${line}`).join("\n");
 ...
 */
async function writeVitestConfig() {
...
async function writeTsconfig() {
  ...
  const tsconfig = {
    ...
    include: ["**/*.ts"],
    compilerOptions: {
      // Ensure tests can use vitest globals & imports without extra config.
...
(async () => {
  try {
    ...
    const generatedFiles = await generateTests(docsWithBlocks);
